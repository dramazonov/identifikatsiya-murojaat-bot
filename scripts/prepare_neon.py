...
_SAFE_MESSAGES = frozenset({
    ...
    "Local migration head differs from c3e9f7a42d11",
    ...
    try:
        ...
        if not resume_validation:
            ...
            with stage(3):
                ...
                if ScriptDirectory.from_config(cfg).get_heads() != ["c3e9f7a42d11"]:
                    raise RuntimeError("Local migration head differs from c3e9f7a42d11")
                await migrate()
        with stage(4):
            from app.database import verify_schema
            await verify_schema()
            async with engine.connect() as conn:
                if (await conn.exec_driver_sql("SELECT version_num FROM alembic_version")).scalar_one() != "c3e9f7a42d11":
